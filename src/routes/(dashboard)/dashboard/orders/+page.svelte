					...
					{#each filtered as order (order.id)}
						...
						>
							...
							<div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl {statusCfg[order.status].bg}">
								...
								{:else if order.status === 'processing'}<RotateCcw size={18} class={statusCfg.processing.text} />
								...
								<div class="min-w-0 flex-1">
									<div class="flex items-center gap-2">
										...
										<span class="rounded-full px-2 py-0.5 text-[10px] font-bold {statusCfg[order.status].bg} {statusCfg[order.status].text}">{statusCfg[order.status].label}</span>
									</div>
									<div class="mt-0.5 flex items-center gap-1.5 text-xs text-[#78716c]">
										<MapPin size={11} /> {order.table}{order.location ? ` · ${order.location}` : ''}
									</div>
									<p class="mt-1.5 text-xs text-[#78716c]">{order.items.map(i => `${i.name} x${i.qty}`).join(', ')}</p>
								</div>
								<div class="shrink-0 text-right">
									<p class="font-bold text-[#1a1a2e]">{formatPrice(order.total)}</p>
									<p class="mt-0.5 flex items-center justify-end gap-0.5 text-[11px] text-[#a8a29e]"><Clock size={9} /> {order.time}</p>
								</div>
							</div>
						</button>
					{/each}
				...
				<div class="p-5 space-y-5">
					...
					<div class="space-y-2">
						<div class="flex items-center justify-between">
							...
							<span class="rounded-full px-3 py-1 text-xs font-bold {statusCfg[selectedOrder.status].bg} {statusCfg[selectedOrder.status].text}">
					...
					<!-- Items -->
					<div>
						<p class="mb-2 text-xs font-bold uppercase tracking-wide text-[#78716c]">Item Pesanan</p>
						<div class="space-y-2">
							{#each selectedOrder.items as item}
								<div class="flex items-start gap-2 rounded-xl bg-[#fafaf9] p-3">
									<span class="flex h-6 w-6 shrink-0 items-center justify-center rounded-lg bg-[#d1fae5] text-xs font-bold text-[#059669]">{item.qty}</span>
									<div class="min-w-0">
					...
					{:else}
						<div class="flex items-center justify-center gap-2 rounded-xl bg-[#fef2f2] py-3 text-sm font-semibold text-[#dc2626]">
