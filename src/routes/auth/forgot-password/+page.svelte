...
<svelte:head>
	<title>Reset your password - Ainything</title>
</svelte:head>
	...
	<div class="app-container grid min-h-[calc(100vh-80px)] place-items-center">
		<section class="surface w-full max-w-lg rounded-lg p-5 text-center sm:p-7">
			{#if sent}
				...
				<h1 class="mt-4 text-2xl font-semibold text-ainything-text">Check your email</h1>
				<p class="mt-3 text-sm leading-6 text-ainything-subtle">
					If <strong>{sentEmail}</strong> is registered, we've sent a password reset link. Check your
					inbox and spam folder.
				</p>
				<a
					href={resolve('/login')}
					class="tap-target mt-6 inline-flex items-center gap-2 rounded-lg bg-ainything-primary px-4 py-2.5 text-sm font-semibold text-white transition-colors hover:brightness-95"
				>
					Back to sign in
				</a>
			{:else}
				<!-- Request form -->
				<span class="inline-flex rounded-lg bg-ainything-primary-soft p-3 text-ainything-primary">
					<Mail size={26} />
				</span>
				<h1 class="mt-4 text-2xl font-semibold text-ainything-text">Forgot your password?</h1>
				<p class="mt-2 text-sm leading-6 text-ainything-subtle">
				...
				<a
					href={resolve('/login')}
					class="tap-target mt-5 inline-flex items-center justify-center gap-1.5 text-sm text-ainything-subtle hover:text-ainything-text"
				>
