<script lang="ts">
	...
	import { ArrowLeft, Minus, Plus, Trash2, ShoppingCart, Send, AlertCircle, Phone } from '@lucide/svelte';
	...
	let {
		data,
		form
	}: { data: PageData; form: { error?: string; success?: boolean; orderId?: string; orderNumber?: number } | undefined } =
		$props();
		...
		{#if showWhatsappField}
			<div>
				<label for="buyer-whatsapp" class="text-xs font-medium text-ainything-subtle">
					Nomor WhatsApp {#if requireWhatsapp}<span class="text-ainything-danger">*</span>{:else}<span class="text-ainything-subtle">(opsional)</span>{/if}
				</label>
				<div class="relative mt-1">
					<Phone size={14} class="pointer-events-none absolute top-1/2 left-3 -translate-y-1/2 text-ainything-subtle" />
					<input
						id="buyer-whatsapp"
						type="tel"
						inputmode="tel"
				...
				<p class="mt-1 text-xs text-ainything-subtle">
					{requireWhatsapp ? 'Untuk konfirmasi pesanan dari pemilik toko.' : 'Isi untuk dapat notifikasi status pesanan via WhatsApp.'}
				</p>
			</div>
		{/if}
		</div>
