...
import { getSessionUser, getDemoSessions, sessionCookieName } from './mock-session';
...
export class MockAuthProvider implements AuthProvider {
	async getSessionUser(cookies: Cookies): Promise<AuthUser | null> {
	...
	async login(email: string, _password: string, cookies: Cookies): Promise<AuthUser> {
		...
		if (!user) {
			throw new Error(
				'Demo user not found. Try owner@bali-table.test or staff@jakarta-hospitality.test'
			);
		}
